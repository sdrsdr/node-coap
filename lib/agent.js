...
Agent.prototype._init = function initSock(socket) {
  ...
  let init_sock=true;
  if (this._opts.fromsrvsock){
	  this._opts.socket=this._opts.fromsrvsock
	  this._sock=this._opts.fromsrvsock;
	  init_sock=false;
  }
...
Agent.prototype._handle = function handle(packet, rsinfo, outSocket) {
  var buf
    , response
    , that = this
