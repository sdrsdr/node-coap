...
CoAPServer.prototype.listen = function(port, address, done) {
  ...
  this._sock.bind(port, address || null, function () {
	...
	if (that._options.mkagent===true) {
		that.agent=new Agent({fromsrvsock:that._sock});
	}
    if (done) done()
  })
  this._port = port
...
CoAPServer.prototype.close = function(done) {
  ...
  if (this.agent) this.agent._cleanUp();
...
CoAPServer.prototype._handle = function(packet, rsinfo) {
  ...
  if (packet.code[0] !== '0') {
	...
	if (this.agent) {
		console.log("non req packet handled in server's agent..");
		let outSocket = this._sock.address();
		this.agent._handle(packet,rsinfo,outSocket);
	} 
    return
  }
  ...
  response = new Message(packet, function(response, packet) {
    ...
    try {
      buf = generate(packet)
		...
		console.log("exception @ new Message :",err);
